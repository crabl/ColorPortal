...
def execute(filename, fpath, color_deficit='d'):
    modified_filename = "%s-%s" % (color_deficit, filename)
...
if __name__ == '__main__':
    ...
    extpos = sys.argv[2].rfind(".")
    if not (extpos > 0 and sys.argv[2][extpos:].lower() in ['.gif', '.jpg', '.jpeg', '.png', '.bmp', '.ico', ]):
    ...
    for col_deficit in [sys.argv[1]]:
        print "Creating %s corrected version" % colorblindness[col_deficit]
        modified_filename, modified_fpath = execute(fname, sys.argv[2], col_deficit)
        if modified_fpath == sys.argv[2]:
